...
export default function ProductionMovementHistory() {
  ...
  const productionMovements = useMemo(
    () => movements.filter((m) => m.area === "produccion"),
    [movements],
  );

  const brands = useMemo(
    () => Array.from(new Set(productionMovements.map((m) => m.brand))).sort(),
    [productionMovements],
  );

  const filtered = useMemo(() => {
    return productionMovements.filter((m) => {
  ...
  }, [productionMovements, brandFilter, catFilter, kindFilter, search]);

  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-base">
          <History className="h-5 w-5 text-primary" /> Historial de movimientos — Producción
        </CardTitle>
        <p className="text-xs text-muted-foreground">
          Entradas y salidas registradas por el área de Producción. Las finalizaciones de cuerpos quedan
          como "Pendiente recepción" hasta que Inventarios las confirma.
        </p>
      </CardHeader>
      <CardContent className="space-y-3">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
          <Input placeholder="Buscar ítem, fabricante, motivo..." value={search} onChange={(e) => setSearch(e.target.value)} />
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>Solicita / Fabrica</TableHead>
