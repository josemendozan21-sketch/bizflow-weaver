...
/* ------------------------------------------------------------------ *
 * Puente "vendido en el mes" vs "liquidado en el mes"
 *
 * El período oficial es la fecha de factura, así que el total que se
 * liquida en un mes NO es lo que el asesor vendió en ese mes: incluye
 * ventas de meses anteriores que se facturaron ahora y deja afuera
 * ventas de este mes que se facturarán después. Esta función explica
 * exactamente de dónde viene la diferencia.
 * ------------------------------------------------------------------ */
...
export function periodLabel(key: string): string {
  ...
  if (!y || !m || !MONTHS_ES[m - 1]) return key;
...
export interface PeriodBridgeBucket extends PeriodTotals {
  /** Periodo de origen o destino en formato YYYY-MM */
  period: string;
  label: string;
}
