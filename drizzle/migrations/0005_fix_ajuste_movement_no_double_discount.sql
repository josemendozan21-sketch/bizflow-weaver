...
BEGIN
  ...
  IF kind = 'ajuste' THEN
    -- Movimiento solo de registro: el ajuste de cantidades ya lo hizo quien lo originó
    RETURN NEW;

  ELSIF kind = 'entrada' THEN
  ...
  ELSIF kind = 'salida' THEN
    ...
    ELSIF NEW.area IN ('punto_venta','tienda_web') THEN target_role_name := 'inventarios';
