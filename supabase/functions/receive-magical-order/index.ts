...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const expected = Deno.env.get('MAGICAL_WEBHOOK_TOKEN') ?? ''
    ...
    const MAGICAL_WEB_ADVISOR_ID = '00000000-0000-0000-0000-000000000002'
    ...
    // Deduplicación por external_order_id
    if (external_order_id) {
      ...
      if (dupErr) {
        console.error('Dedup lookup error:', dupErr)
      }
      if (existing) {
        return new Response(
    ...
    const order = {
      ...
      advisor_id: MAGICAL_WEB_ADVISOR_ID,
      advisor_name: String(body.advisor_name ?? 'Magical Warmers Web'),
