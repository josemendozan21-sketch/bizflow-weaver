ALTER TABLE public.body_production_tasks ADD COLUMN IF NOT EXISTS brand text NOT NULL DEFAULT 'magical';
ALTER TABLE public.inventory_movements ADD COLUMN IF NOT EXISTS logo text;
ALTER TABLE public.inventory_audit_log ADD COLUMN IF NOT EXISTS logo text;
...
BEGIN
  ...
  IF TG_OP = 'DELETE' THEN
    ...
    rec_brand := OLD.brand;
    IF TG_TABLE_NAME = 'stock_items' THEN
      rec_name := OLD.name; rec_cat := OLD.category; rec_type := OLD.product_type; rec_logo := OLD.logo;
    ELSE
  ...
  rec_brand := NEW.brand;
  IF TG_TABLE_NAME = 'stock_items' THEN
    rec_name := NEW.name; rec_cat := NEW.category; rec_type := NEW.product_type; rec_logo := NEW.logo;
  ELSE
